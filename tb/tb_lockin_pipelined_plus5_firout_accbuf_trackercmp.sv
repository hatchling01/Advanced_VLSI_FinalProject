...
module tb_lockin_pipelined_plus5_firout_accbuf_trackercmp;
    ...
    localparam int ENERGY_WIDTH = 128;
    ...
    lockin_pipelined_plus5_firout_accbuf_trackercmp_top #(
    ...
    initial begin
        ...
        wait_cycles = 0;
        while (!done && wait_cycles < 2000) begin
            @(posedge clk);
            wait_cycles++;
        end
        ...
        if (!done) begin
            $fdisplay(result_fd, "FAIL: timeout waiting for done");
            $fclose(result_fd);
            $fatal(1, "FAIL: timeout waiting for done");
        end
        ...
        if ($unsigned(detected_bin) == expected_bin) begin
            $display("PASS");
